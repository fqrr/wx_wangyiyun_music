  ...
  onLoad: function (options) {
   ...
   async getVideoGroupListData(){
        ...
        this.setData({
          ...
          // 截取第一个数据的id 让它默认选中第一个
          navId:videoGroupListData.data[0].id
        })
        // 不放在onload是因为执行一次 而初始化时只是一个空串 ，放在这就可以取到值再传进getVideoList()函数里
        this.getVideoList(this.data.navId)//已经在date中存储
    ...
    //------------------------
      ...
      // 点击切换导航的回调
      changNav(event){
        // 之前已经给元素绑定过id ,所以现在可以获取事件目标的id
            let navId=event.currentTarget.id//通过id向event传参的时候如果传的是number会自动转换成string
            // let navId=event.currentTarget.dataset.id
            this.setData({
              navId:navId>>>0,
              // 因为之前的值是number 而现在的navId是string,所以要把string转成number
              // 跳转到其他标签页时，其他页面视频应该隐藏
              videoList:[]
            })
              //显示加载
              wx.showLoading({
...
// 点击播放/继续播放的回调
      ...
      // 刚开始播放时是没有videoContext这个实例对象 所以不需要调用this.videoContext.stop()
      // this.vid !==vid 是让视频停止后可以继续播放 而不调用stop
    //  this.vid !==vid && this.videoContext && this.videoContext.stop();//此时停的是上一个视频的id
    //  if(this.vid !== vid){
  ...
  handleRefresher(){
        console.log('来了来了');
      // 再次发请求，获取最新数据视频列表数据（也就是再调用获取数据的函数）  // 
      this.getVideoList(this.data.navId);
  }
  ,
  // 自定义上拉触底的回调scrpll-view
  handleToLower(){
    console.log('我到底部拉');
    // 数据分页 1、后端分页  2、前端分页
    // 模拟数据
